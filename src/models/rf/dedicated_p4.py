...
def load_config(fname):
    ...
    config                          = {}
    config['num_features']          = config_path["number of features"]
    config['num_trees']             = config_path["number of trees"]
    config['num_classes']           = config_path["number of classes"]
    config['column_width']          = config_path['width of feature']
    config['result_width']          = config_path['width of result']
    config['code_width']            = config_path['width of code']
    config['feature_table_depth']   = config_path['used columns']
    config['headers_list']          = config_path['standard headers']
    config['code_tbl_depth']        = config_path['code table size']
    config["decision_table_size"]   = config_path["decision table size"]
    config['probability_width']     = config_path['width of probability']
    config['model']                 = config_path['model']
    config['default label']         = config_path["default label"]
    config['default_vote']          = config_path["default vote"]
...
def create_tables_command(fname, config):
    ...
    with open(fname, 'w') as file:
        for f in range(num_features):
            for idx in table_ternary['feature ' + str(f)]:
                priority    = int(idx)
                key         = table_ternary['feature ' + str(f)][idx][1]
                mask        = table_ternary['feature ' + str(f)][idx][0]
                codes       = ''

                for t in range(num_trees):
                    c_tree  = table_ternary['feature ' + str(f)][idx][2][t]
                    c_len   = config['p4 config']['width of code'][t][f]
                    codes   = ten_to_bin(int(c_tree), int(c_len)) + codes

                label = int(codes, 2)
                file.write("table_add SwitchIngress.lookup_feature"     \
                           + str(f) + " extract_feature" + str(f)       \
                           + " " + str(key) + "&&&" + str(mask)         \
                           + " => " + str(label) + " "                  \
                           + str(priority) + "\n")
        ...
        for t in range(num_trees):
            for idx in table_ternary['tree ' + str(t)]:
                file.write("table_add SwitchIngress.lookup_leaf_id" + str(t) \
                           + " read_prob" + str(t) + " ")
                for f in range(num_features):
                    file.write(str(table_ternary['tree ' + str(t)][idx]['f' + str(f) + ' code']) \
                               + " ")
                file.write("=> 0 " + str(table_ternary['tree ' + str(t)][idx]['leaf']) + "\n")

            file.write("\n")

        for idx in table_ternary['decision']:
...
def create_load_tables(fname, fjson, config, planter_config, file_name):
    ...
    with open(fname, 'a') as tload:
        ...
        #  Load feature tables
        # for i in range(0, config['num_features']):
        #     tload.write("print('load feature " + str(i) + " table with',len(table['feature " + str(
        #         i) + "'].keys()),'entries')\n" \
