...
def load_config(fname):
    ...
    config['num_bits'] = config_file["action data bits"]
    config['num_hps'] = config_file["number of hps"]
    config['f_tbl_len'] = config_file["feature tbl len"]
    config['thresh_and_bias'] = config_file['thresh and bias']
    config['hp_info'] = config_file['hp_info']
...
def create_tables(planter_config):
    ...
    for idx in Exact_Table['decision']:
        ...
        for c in range(num_classes):
            Entry["match"]["meta.vote_c"+str(c)] = int(Exact_Table['decision'][idx]['c'+str(c)+' vote'])
        Entry["action_name"] = "SwitchIngress.read_lable"
        Entry["action_params"] = {}
        Entry["action_params"]["label"] = int(Exact_Table['decision'][idx]["class"])
        Table_entries += [Entry]
    ...
    Entry["action_params"] = {}
    for hp in range(num_hps):
        Entry["action_params"]["thresh_hp"+str(hp)] = int(Exact_Table['threshold hp'+str(hp)])
        Entry["action_params"]["bia_hp" + str(hp)] = int(Exact_Table['bias hp' + str(hp)])
    Table_entries += [Entry]
...
def create_tables_command(fname, config):
    ...
    with open(fname, 'w') as file:
        ...
        for hp in range(num_hps):
            file.write(str(int(Exact_Table['threshold hp' + str(hp)]))+" ")
        for hp in range(num_hps):
            file.write(str(int(int(Exact_Table['bias hp' + str(hp)]))) + " ")
        file.write("\n")
...
def create_load_tables(fname, fjson, config, planter_config):
    ...
    # command_file = work_root + "/Tables/s1-commands.txt"
