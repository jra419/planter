...
def run_model(train_X, train_y, test_X, test_y, used_features):
    ...
    for t in range(num_trees):
        ...
        print('')
        print('Begine transfer: Tree '+str(t)+' table ')
        key_name = []
        for f in range(num_features):
            key_name += ['f' + str(f) + ' code']
        action_name = 'leaf'
        # prepare default
        LPM_Table['tree '+str(t)] = Muti_Exact_to_LPM_Concatination(Exact_Table['tree '+str(t)], code_width_tree_feature[t], key_name, action_name)

    # ===================== decision table to LPM =========================
    default_class = 0
    ...
    decision_table_key_width = []
    for t in range(num_trees):
        # decision_table_key_width += [5]
        decision_table_key_width += [int(1+np.ceil(math.log(num_classes, 2)))]

    # prepare default
    LPM_Table['decision'] = Muti_Exact_to_LPM_Concatination(Exact_Table['decision'], decision_table_key_width, key_name, action_name)
    ...
    json.dump(LPM_Table, open('Tables/'+table_name, 'w'), indent=4)
    print('\nLPM_Table is generated')
    ...
    Planter_config['p4 config']["width of decision table keys"] = decision_table_key_width
...
def test_tables(sklearn_test_y, test_X, test_y):
    ...
    num_depth = Planter_config['model config']['number of depth']
    max_leaf_nodes = Planter_config['model config']['max number of leaf nodes']
    decision_table_key_width = Planter_config['p4 config']["width of decision table keys"]
    ...
    for i in range(np.shape(test_X.values)[0]):
        ...
        for tree in range(num_trees):
            ...
            if str(code_list)!=str(lpm_code_list):
                print('error in exact to lpm match', code_list, lpm_code_list)

            binary_code = ''
            for f in range(num_features):
                binary_code = binary_code + ten_to_bin(int(code_list[f]), int(Planter_config['p4 config']["width of code"][tree][f]))
            decimal_code = int(binary_code, 2)

            LPM_table = LPM_Table['tree '+str(tree)]
            keys = list(LPM_table.keys())
            mask = []
            action = []
            for count in np.sort(keys):  # For each value in LPM table, check if it matches that separation key
                if decimal_code & LPM_table[count][0] == LPM_table[count][0] & LPM_table[count][1]:  # if there is a ternary match
                    mask.append(LPM_table[count][0])  # array of masks
                    action.append(LPM_table[count][2])  # array of actions
            max_mask = max(mask)
            max_index = mask.index(max_mask)
            vote_list[tree] = action[max_index]

        binary_code = ''
        for t in range(num_trees):
            binary_code = binary_code + ten_to_bin(int(vote_list[t]), decision_table_key_width[t])
        decimal_code = int(binary_code, 2)

        LPM_table = LPM_Table['decision']
        keys = list(LPM_table.keys())
        mask = []
        action = []
        for count in np.sort(keys):  # For each value in LPM table, check if it matches that separation key
            if decimal_code & LPM_table[count][0] == LPM_table[count][0] & LPM_table[count][
                1]:  # if there is a ternary match
                mask.append(LPM_table[count][0])  # array of masks
                action.append(LPM_table[count][2])  # array of actions
        max_mask = max(mask)
        max_index = mask.index(max_mask)
        switch_prediction = action[max_index]
    ...
    print('Exit, the P4 generator (dedicate P4) is currently unavailable in this variation.')
    exit()
...
def resource_prediction():
    ...
    print('LPM match entries: ', np.sum(Planter_config['p4 config']["used columns"]))
