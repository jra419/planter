...
def run_model(train_X, train_y, test_X, test_y, used_features):
    ...
    # =================== set tree numbers in config ===================
    question = 'Number of trees?'
    default = 5
    Planter_config = take_CLI_input(Planter_config, 'model config', 'number of trees', question, default,
                                    manually_input = False, numeric=True)

    # =================== set depth in config ===================
    question = 'Number of depth?'
    default = 4
    Planter_config = take_CLI_input(Planter_config, 'model config', 'number of depth', question, default,
                                    manually_input = False, numeric=True)

    # =================== set max leaf node in config ===================
    question = 'Number of leaf nodes?'
    default = 1000
    Planter_config = take_CLI_input(Planter_config, 'model config', 'max number of leaf nodes', question, default,
                                    manually_input = False, numeric=True)
    ...
    Planter_config['p4 config']["decision table size"] = len(Ternary_Table['decision'].keys())+1
    Planter_config['p4 config']["code table size"] = []
    for tree in range(num_trees):
        Planter_config['p4 config']["code table size"] += [len(Ternary_Table['tree '+str(tree)].keys())+1]
    ...
    for i in range(num_features):
        Planter_config['p4 config']["used columns"] += [len(Ternary_Table['feature '+str(i)].keys())+1]
