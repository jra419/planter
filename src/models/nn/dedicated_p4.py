...
def load_config(fname):
    ...
    config['num_hidden_nodes'] = config_path['num hidden nodes']
    config['num_layers'] = config_path["number of layers"]
    ...
    with open(fname, 'a') as ingress:
        ...
        for f in range(config['num_features']):
            if f+1<config['num_features']:
                ingress.write("        meta.bnnInput = (meta.bnnInput + (bit <64>) meta.feature"+str(f)+") << "+str(int(config['width'][f+1]))+";\n")
            else:
